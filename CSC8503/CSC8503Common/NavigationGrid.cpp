...
NavigationGrid::NavigationGrid(const std::string&filename) : NavigationGrid() {
	...
	for (int y = 0; y < gridHeight; ++y) {
		for (int x = 0; x < gridWidth; ++x) {
			...
			{
			case '.':
				n.gridType = GRIDTYPE::Blank;
				break;
			
			case 'x':
				n.gridType = GRIDTYPE::Block;
				break;
...
bool NCL::CSC8503::NavigationGrid::ReachedWayPoint(Vector3 position, Vector3 wayPoint)
{
	int positionX = ((int)position.x / nodeSize);
	int positionZ = ((int)position.z / nodeSize);

	int wayPointX = ((int)wayPoint.x / nodeSize);
	int wayPointZ = ((int)wayPoint.z / nodeSize);

	float distance = Vector3((wayPoint.x - position.x), 0, (wayPoint.z - position.z)).Length();

	if ((positionX == wayPointX && positionZ == wayPointZ)|| distance < nodeSize/2) {
		return true;
	}
	return false;
}
